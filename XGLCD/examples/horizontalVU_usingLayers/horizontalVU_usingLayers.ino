/*
An old school example of how to use layers and graphic boolean operations
for fast displaying several vu meter bars.
The Colored bars are created once all together, they are masked by an AND
operation by 2 different rectangles, one which act as passTrough from the original point to the value,
another black rectangle starting from the value to the end of the colored bar.
As result the operation is really fast even on a slow SPI.
...
void drawHbarVal(uint16_t originX, uint16_t originY, uint8_t segments, uint8_t maxVal) {
  //tft.fillRect(originX+segW*(2*segments-1)+2,originY+2,segW,barH-4,COLOR_WHITE);//single dot
  tft.fillRect(originX + 2, originY + 2, segW * (2 * segments - 1), barH - 4, COLOR_WHITE);
  tft.fillRect(originX + 2 + segW * (2 * segments - 1), originY + 2, (2 + segW * (2 * maxVal - 1)) - (segW * (2 * segments - 1) + 2), barH - 4, COLOR_BLACK);
