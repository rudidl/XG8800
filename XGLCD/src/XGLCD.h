...
/* TODO
- expand library to make it compatible wiht Adafruit calls (GFX), TFT_eSPI/eFEX, GuiSlice, Littlevgl)
- make all variable 32-bit to speed things up
- test everything with all cpu modules
*/
...
// coordinate reads. The read speed should never be higher then 4Mhz.
    ...
    #define SPI_SPEED_WRITE                 20000000
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
//         EXTERNAL LIBRARIES COMPATIBILITY LAYERS                             +
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

// UTFT compatibility layer
#define InitLCD(a)
#define clrScr() clearScreen(COLOR_BLACK)
#define fillScr(r, g, b) clearScreen(Color565(r, g, b))
#define fillScr(color) clearScreen(color)
#define lcdOff() displayOn(false)
#define lcdOn() displayOn(true)
#define setContrast(a)
#define getDisplayXSize() _width
#define getDisplayYSize() _height
#define setBrightness(a) brightness(a)
#define drawHLine(x, y, w) drawFastHLine(x, y, w, _foreColor);
#define drawVLine(x, y, h) drawFastVLine(x, y, h, _foreColor);
#define getFontXSize() getFontWidth(false)
#define getFontYSize() getFontHeight(false)
#define setDisplayPage(page)
#define setWritepage(page)

// Adafruit compatibility layer
#define textMode() _setTextMode(true)
#define graphicsMode() _setTextMode(false)
#define textSetCursor(x, y) setCursor(x, y)
#define textColor(foreColor, bgColor) setTextColor(foreColor, bgColor)
#define textTransparent(foreColor) setTextColor(foreColor)
#define textEnlarge(scale) setFontScale(scale)
#define textWrite(buffer, len) write(*buffer, len)
#define fillScreen(color) clearScreen(color)
#define touchRead(x, y) touchReadPixel(*x, *y)
#define PWM1config(on, clock) PWMsetup(1, on, clock)
#define PWM2config(on, clock) PWMsetup(2, on, clock)
#define PWM1out(p) PWMout(1, p)
#define PWM2out(p) PWMout(2, p)
...
class XGLCD : public Print {
  public:
    ...
    void        drawRoundRect(int16_t x0, int16_t y0, int16_t x1, int16_t y1);
    void        drawRoundRect(int16_t x, int16_t y, int16_t w, int16_t h, int16_t r, uint16_t color);
    void        fillRoundRect(int16_t x0, int16_t y0, int16_t x1, int16_t y1);
    ...
    // GUI Elements
    void         ringMeter(int val, int minV, int maxV, int16_t x, int16_t y, uint16_t r, const char* units="none", uint16_t colorScheme=4,uint16_t backSegColor=COLOR_BLACK,int16_t angle=150,uint8_t inc=10);
    void        roundGaugeTicker(uint16_t x, uint16_t y, uint16_t r, int from, int to, float dev,uint16_t color);

    void        drawBitmap(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t * image, uint16_t scale);
    void        drawBitmap(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t * image, uint16_t degree, uint16_t rx, uint16_t ry);
    ...
    void        setTextGradient(uint16_t fcolor1,uint16_t fcolor2);
    ...
    void        BTE_layer(enum RA8875btelayer sd,uint8_t l);
...
#define calibrateRead()
    ...
    virtual size_t write(uint8_t b) {
        _FNTgradient = false;                                                   //cant use this with write
...
protected:
    ...
    bool        _TXTrender;
    bool        _TXTAlignXToCenter;
    bool        _TXTAlignYToCenter;
    bool        _TXTwrap;
    bool        _FNTgradient;
    uint16_t    _FNTgradientColor1;
    uint16_t    _FNTgradientColor2;
...
private:
    ...
    // TEXT writing stuff
    void        _textWrite(const char* buffer, uint16_t len=0);
    ...
    void        _fillTriangleBottomFlat(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color);
    void        _fillTriangleTopFlat(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint16_t color);
    ...
    void        _waitBusy(uint8_t res=0x80);                                    //0x80, 0x40(BTE busy), 0x01(DMA busy)
     inline __attribute__((always_inline)) void _checkLimits_helper(int16_t &x,int16_t &y);
    inline __attribute__((always_inline)) void _center_helper(int16_t &x, int16_t &y);
