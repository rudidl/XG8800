/*
Basic Text Functions (only XGLCD internal fonts)
Here's an example of how to use text commands
...
void setup()
{
  ...
  tft.fillWindow(COLOR_BLACK);//fill window black
  //now set a text color, background transparent
  tft.setTextColor(COLOR_WHITE);
  Sprintln(tft.getFontWidth(true));
  Sprintln(tft.getFontHeight(true));
  ...
  tft.drawPixel(currentX, currentY, COLOR_WHITE); //did you see the white dot?
  ...
  tft.setCursor(CENTER, CENTER); //this set text exact in the screen regardless his length and scale
  tft.setFontScale(1);//font x2
  tft.setTextGradient(COLOR_YELLOW,COLOR_MAGENTA);
  tft.print("I'm in the center");
  //now use the autocenter feature
  tft.setFontScale(0);//reset font scale
  tft.drawRect((tft.width() / 2) - 25, (tft.height() / 2) - 100, 50, 50, COLOR_RED);//draw a box above the center
  tft.setTextColor(COLOR_YELLOW);
