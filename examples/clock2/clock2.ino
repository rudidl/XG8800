/*
A similar clock example but this time uses some XGLCD gauges library function!
...
const uint16_t clockColors[5] = {COLOR_BLACK, COLOR_BLACK, COLOR_CYAN, COLOR_BLUE, COLOR_RED};
...
void drawGauge(int16_t pos[], uint16_t from, uint16_t to) {
  ...
  tft.roundGaugeTicker(pos[0], pos[1], pos[2], from, to, 1.3, COLOR_WHITE);
  if (pos[2] > 15) tft.roundGaugeTicker(pos[0], pos[1], pos[2], from + 15, to - 15, 1.1, COLOR_WHITE); //draw minor ticks
}
