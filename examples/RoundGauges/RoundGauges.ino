/*
The simpliest way to draw a Round Gauge by using the XGLCD library functions.
...
void loop(void) {
  ...
  gaugeVal1[1] = drawNeedle_(gaugeVal1, gaugePos1, gaugeLimits1, COLOR_RED);
  gaugeVal2[1] = drawNeedle_(gaugeVal2, gaugePos2, gaugeLimits2, COLOR_GREEN);
  gaugeVal3[1] = drawNeedle_(gaugeVal3, gaugePos3, gaugeLimits3, COLOR_BLUE);
  delay(150);
}
...
int16_t drawNeedle_(volatile int16_t val[], const int pos[], const int limits[], uint16_t color) {
  if (val[0] != val[1]) {
    ...
    tft.drawLineAngle(pos[0], pos[1], map(val[1], 0, 1023, 0, angleMax), len, COLOR_BLACK, limits[0]);
...
void drawGauge(const int pos[], const int limits[]) {
  tft.drawCircle(pos[0], pos[1], pos[2], COLOR_WHITE); //draw instrument container
  tft.drawCircle(pos[0], pos[1], pos[2] + 1, COLOR_WHITE); //draw instrument container
  tft.roundGaugeTicker(pos[0], pos[1], pos[2], limits[0], limits[1], 1.3, COLOR_WHITE);
  if (pos[2] > 15) tft.roundGaugeTicker(pos[0], pos[1], pos[2], limits[0] + 15, limits[1] - 15, 1.1, COLOR_WHITE); //draw minor ticks
}
