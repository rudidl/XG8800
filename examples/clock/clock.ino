...
void drawClockFace() {
  tft.fillCircle(ccenterx, ccentery, cradius, COLOR_BLUE);
  tft.fillCircle(ccenterx, ccentery, cradius - 4, COLOR_BLACK);
  // Draw 12 lines
  for (int i = 0; i < 360; i += 30) {
    ...
    tft.drawLine(x0, yy0, x1, yy1, COLOR_BLUE);
  }
}
...
void drawClockHands(uint8_t h, uint8_t m, uint8_t s) {
  ...
  tft.drawLine(ohx, ohy, ccenterx + 1, ccentery + 1, COLOR_BLACK);
  tft.drawLine(omx, omy, ccenterx + 1, ccentery + 1, COLOR_BLACK);
  tft.drawLine(osx, osy, ccenterx + 1, ccentery + 1, COLOR_BLACK);
  // Draw new hand positions
  tft.drawLine(hx * (cradius - 28) + ccenterx + 1, hy * (cradius - 28) + ccentery + 1, ccenterx + 1, ccentery + 1, COLOR_WHITE);
  tft.drawLine(mx * (cradius - 17) + ccenterx + 1, my * (cradius - 17) + ccentery + 1, ccenterx + 1, ccentery + 1, COLOR_WHITE);
  tft.drawLine(sx * (cradius - 14) + ccenterx + 1, sy * (cradius - 14) + ccentery + 1, ccenterx + 1, ccentery + 1, COLOR_RED);
  tft.fillCircle(ccenterx + 1, ccentery + 1, 3, COLOR_RED);
