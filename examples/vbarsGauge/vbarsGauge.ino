/*
This example creates a huge vertical bars gauge display with more than 230 bars!
Perfect for FFT, etc. 
...
will reveal the background) in the layer 2 by using AND function. The result is a really
fast huge vertical bars gauge that it's not possible to do with a tiny microcontroller
...
boolean createBar(uint16_t origX, uint16_t origY) {
  ...
  for (i = 0; i < _segments; i++) {
    ...
    if (i > 0 && i < (_segments-1)) {
      ...
      tft.fillRect(origX, newOriginY + ((_segHeight * i) + (i * 2)), _barWidth, _segHeight, COLOR_YELLOW);//TOP
    } else {
      tft.fillRect(origX, newOriginY + ((_segHeight * i) + (i * 2)), _barWidth, _segHeight, COLOR_WHITE);//BOTTOM
    }
  }
  return true;
}
